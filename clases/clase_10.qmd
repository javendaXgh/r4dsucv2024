...
## Funciones `slice` y sus varianzas
...
## equivalente `slice_min()` en Rbase
```{r}
df_gapminder%>%
  filter(country %in% c('Colombia','Venezuela'))%>%
...
```
aplica usar `n` igual que `slice_min()`

## equivalente `slice_max()` en Rbase
```{r}
df_gapminder%>%
  filter(country %in% c('Colombia','Venezuela'))%>%
...
## equivalente `slice_head()` en Rbase
```{r}
df_gapminder%>%
  filter(continent==continentes[2])%>%
  .[1:10, ]
...
## equivalente `slice_tail()`  en Rbase
```{r}
df_gapminder%>%
  filter(continent==continentes[3])%>%
  .[(nrow(.)-10):nrow(.), ]
